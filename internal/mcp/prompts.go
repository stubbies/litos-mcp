...
5. **Find callers** — After reading a symbol, call find_callers with its name (exact, case-sensitive) or symbol_id to see who invokes it. Use enclosing_symbol_id from each hit to read_symbol on the caller and iterate up the call chain.
6. **Prefer litos over grep** — When litos-mcp tools are available, use search_code_skeleton, outline_file, read_symbol, and find_callers instead of grep, ripgrep, semantic search over raw files, or Read on entire source files. Grep is a fallback only when litos returns no hits or you need exact string/regex matches litos cannot express.
7. **Iterate** — Refine query keywords from symbol names, kinds, scopes, matched_in, and symbol_id fields until you locate the implementation you need. If read_symbol reports a stale symbol_id after edits, re-search or re-outline for a fresh ID. If find_callers returns no hits, the indexed callee name may differ — try search_code_skeleton first.
...
func registerPrompts(server *mcpsdk.Server) {
	server.AddPrompt(&mcpsdk.Prompt{
		...
		Description: "Guidance for discover → read_symbol → find_callers workflow with litos-mcp instead of grep or whole-file reads.",
...
func handleCodeDiscoveryWorkflow(_ context.Context, req *mcpsdk.GetPromptRequest) (*mcpsdk.GetPromptResult, error) {
	...
	return &mcpsdk.GetPromptResult{
		Description: "Prefer litos-mcp search, outline, read_symbol, and find_callers over grep or whole-file reads.",
